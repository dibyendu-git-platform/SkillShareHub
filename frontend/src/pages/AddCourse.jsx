...
const AddCourse = () => {
    ...
    const onSubmit = data => {
        ...
        formData.append('courseThumbnail', data.thumbnail[0]); // Assuming thumbnail is a file input
        formData.append('published', data.published);

        axios.post(`${import.meta.env.VITE_BASE_URL}/api/v1/course/create}`, formData, {
            headers: {
        ...
        <div className="max-w-4xl mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
            ...
            <form className="space-y-5" id="courseForm" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div>
                    ...
                    <input type="file" name="thumbnail" accept="image/*" className="w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:border file:border-gray-300 file:rounded-lg file:bg-gray-100 file:text-sm file:font-semibold file:text-gray-700 hover:file:bg-gray-200" {...register("thumbnail", { required: "this field is required"})}/>
                    {errors.thumbnail && <p role="alert" className='text-red-600'>{errors.thumbnail?.message}</p>}
                </div>
